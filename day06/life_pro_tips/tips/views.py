...
def deltip(request):
    if request.user.is_authenticated:
        tip = Tip.objects.select_related('author').filter(id=request.POST.get("tip_id")).first()
        if tip:
            if tip.author == request.user or request.user.has_perm("tips.delete_tip") or request.user.can_delete_tips() :
                with transaction.atomic():
                    up_loss = tip.upvotes.count() * 5
                    down_gain = tip.downvotes.count() * 2
                    tip.author.reputation = F('reputation') - up_loss + down_gain
                    tip.author.save()
                    tip.delete()
    return (redirect('/'))
    ...
    if request.user.is_authenticated:
        ...
        if tip and (request.user.has_perm("tips.candownvote") or user == tip.author or user.can_downvote()):
            if user in tip.upvotes.all():
                ...
                tip.author.reputation = F('reputation') - 5
                tip.author.save()
            if user in tip.downvotes.all():
                tip.downvotes.remove(user)
                tip.author.reputation = F('reputation') + 2
                tip.author.save()
            else:
                tip.author.reputation = F('reputation') - 2
